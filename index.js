...
async function mainMenu(id) {
  try {
    await bot.sendMessage(id, "منوی اصلی", {
      parse_mode: "Markdown",
      reply_markup: {
        keyboard: [[`روش نصب coinomi`,`دربارۀ بیت‌کوین`, "احراز هویت"]],
...
bot.onText(/روش نصب/, async (msg) => {
  try {
    ...
    await bot.sendMessage(msg.from.id,`توجه: در این بخش اگر از قبل کیف پول کوینومی داشته‌اید، می‌توانید با فشردن گزینه‌ی «Restore a wallet» آن را بازیابی کنید. مراحل این کار در پایان توضیح داده خواهد شد.\nقدم سوم
...
توجه: برای ذخیره‌ی عبارت بازیابی، از صفحه‌ی گوشی خود اسکرین شات نگیرید زیرا خود برنامه نیز به شما این اخطار را می‌دهد که روش امنی نخواهد بود.\n✅`);
    ...
    await bot.sendMessage(msg.from.id,`قدم چهارم
پس از آن باید عبارت بازیابی را وارد و دکمه‌ی Next را فشار دهید.\n✅`);
    ...
    await bot.sendMessage(msg.from.id,`قدم پنجم
پسوردی را برای استفاده روزانه‌ی خود انتخاب کنید. این پسورد باید بین ۱۰ تا ۲۸ حرف باشد. سپس، Next را بزنید.\n✅`);
    ...
    await bot.sendMessage(msg.from.id,`قدم ششم
در بخش بعدی شما می‌توانید کوین‌هایی که می‌خواهید بیفزایید و استفاده کنید را انتخاب کنید. حتی اگر نمی‌دانید به غیر از بیت کوین از چه ارز دیجیتال دیگری استفاده خواهید کرد، نگران نباشید، می‌توانید متعاقباً آن‌ها را اضافه کنید.\n✅`);
    await bot.sendMessage(msg.from.id,`قدم هفتم
دکمه‌ی پذیرش (Accept) شرایط و مقررات سرویس‌دهی (Terms of Services) را فشرده و به مرحله‌ی بعد بروید.\n✅`);
    ...
    await bot.sendMessage(msg.from.id,`اگر می‌خواهید ارز دیجیتال جدیدی را اضافه کنید، روی گزینه‌ Coin+ که در پایین صفحه قرار دارد، کلیک کنید.\n✅`);
    await bot.sendPhoto(msg.from.id,`./b8.jpg`);
    await bot.sendMessage(msg.from.id,`در این مرحله، ایجاد کیف پول به پایان می‌رسد. اکنون به بررسی برخی گزینه‌های این کیف پول می‌پردازیم.
یکی از ویژگی‌های خوب این برنامه آن است که می‌توانید با توجه به ارز رایج دلخواهتان، قیمت ارزهای دیجیتال را مشاهده کنید. در این مرحله، پیغام دریافت اجازه برای ارسال هشدار از سوی این کیف پول را مشاهده خواهید کرد که با فشردن Allow این اجازه را به آن بدهید.\n✅`);
    ...
    await bot.sendMessage(msg.from.id,`برای ارسال ارز دیجیتال، از گزینه‌ی سه خطی بالا سمت چپ، روی ارز دیجیتالی که قصد ارسال آن را دارید کلیک کنید و وارد تب ارسال (Send) شوید. در این بخش نیاز است تا آدرس مقصد و میزان مورد نظر برای ارسال را وارد کادرهای مربوطه کنید. از گزینه‌ی Scan QR Code نیز می‌توان استفاده کرد، بدین شکل که باید از بارکد آدرس مقصد عکس بگیرید. گزینه‌ی Use all funds نیز به معنی آن است که می‌توانید کل دارایی‌تان را یکجا ارسال کنید. پس از وارد کردن این اطلاعات، باید از صحت تک تک آن‌ها اطمینان حاصل کنید و سپس دکمه‌ی ارسال را فشار دهید.\n✅`);
    await bot.sendPhoto(msg.from.id,`./b10.jpg`);
    await bot.sendMessage(msg.from.id,`پس از آن که تراکنش را ارسال کردید، باید منتظر تایید آن بمانید. وضعیت این تراکنش را می‌توانید در تب Balance مشاهده کنید. اگر روی تراکنش انجام شده کلیلک کنید، می‌توانید از جزییات آن مطلع شوید. برای مثال می‌توانید از تاریخ انجام، وضعیت آن، میزان جا به جا شده، کارمزد تراکنش و شناسه‌ی تراکنش با خبر شوید.\n✅`);
    await bot.sendMessage(msg.from.id,`برای دریافت ارز دیجیتال از دیگران، دوباره روی همان گزینه‌ی سه خطی کلیک کرده و این بار در منوی باز شده، روی کوین‌هایی که اضافه کرده بودید کلیک کنید. خواهید دید در تب Receive که در پایین قرار دارد، آدرسی برای شما نشان داده خواهد شد که در زیر آن نیز QR کد آن قرار دارد. این آدرس را کپی کرده و به فردی که می‌خواهید برایتان ارز دیجیتال بفرستد، ارسال کنید و یا آن را در صرافی که قصد ارسال ارز دیجیتالتان را از آن دارید، تایپ کنید.\n✅`);
    await bot.sendPhoto(msg.from.id,`./b11.jpg`);
    await bot.sendMessage(msg.from.id,`برای بررسی موجودی کیف پول خود، دوباره همان مراحل بالا را تکرار کنید، اما این بار به جای تب Receive وارد تب Balance شوید. در این بخش تمام تراکنش‌های انجام شده را خواهید دید.\n✅`);
